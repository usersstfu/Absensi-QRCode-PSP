                    ...
                    <form action="{{ route('store-leave-request') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <div>
                                    ...
                                    <x-select id="status" class="mt-1 block w-full" name="status" required>
                                        ...
                                        <option value="sick"
                                            {{ (old('status') ?? $attendance?->status) === 'sick' ? 'selected' : '' }}>
                                            Sakit
                                        </option>
                                        <option value="dinas"
                                            {{ (old('status') ?? $attendance?->status) === 'dinas' ? 'selected' : '' }}>
                                            Perjalanan Dinas
                                        </option>
                                    </x-select>
                                    @error('status')
                                ...
                                <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-3">
                                    <div>
                                        ...
                                        <x-input type="date" min="{{ date('Y-m-d') }}" value="{{ date('Y-m-d') }}"
                                            id="from" class="mt-1 block w-full" name="from" required />
                                        @error('from')
                                    ...
                                    <div>
                                        ...
                                        <x-input type="date" id="to" min="{{ date('Y-m-d') }}"
                                            class="mt-1 block w-full" name="to" />
                                        @error('to')
                            ...
                            <div x-data="{ filename: null, preview: null }">
                                <input type="file" value="{{ old('attachment') ?? $attendance?->attachment }}"
                                    class="hidden" id="attachment" name="attachment" x-ref="attachment"
                                    x-on:change="
                            ...
                            " />
                                ...
                                <x-secondary-button class="me-2 mt-2" type="button"
                                    x-on:click.prevent="$refs.attachment.click()">
